
## [0.4.10] - 2026-07-21

### Fixed
- [fix] update large variant in chip component

---


## [0.4.9] - 2026-07-21

### Fixed
- [fix] export components and interfaces

---


## [0.4.8] - 2026-07-21

### Fixed
- [fix] update font weight in chip component

---


## [0.4.7] - 2026-07-21

### Fixed
- [fix] update padding in chip component

---


## [0.4.6] - 2026-07-21

### Added
- [feat] add useOnline hook

---


## [0.4.5] - 2026-07-21

### Fixed
- [fix] remove aria readonly in chip component, use readonly class

---


## [0.4.4] - 2026-07-21

### Added
- [feat] add size in chip component

---


## [0.4.3] - 2026-07-21

### Fixed
- [fix] readonly correction in chip component

---


## [0.4.2] - 2026-07-21

### Fixed
- [fix] support href in chip component

---


## [0.4.1] - 2026-07-21

### Fixed
- [fix] update chip components styles

---


## [0.4.0] - 2026-07-21

### Added
- [add] implementation chip component

---


## [0.3.2] - 2026-07-21

### Fixed
- [fix] update colors in scrollToTop component

---


## [0.3.1] - 2026-07-21

### Fixed
- [fix] export scrollToTop component

---


## [0.3.0] - 2026-07-21

### Added
- [add] implementation scrollToTop component
- [feat] add introduction in storybook

### Changed
- [update] add preview link in readme

### Fixed
- [fix] update scrollToTop component styles
- [fix] add prefix in vars for styles

---


## [0.2.9] - 2026-07-16

### Changed
- [update] export icon name and declare fontssize default in icon component

---


## [0.2.8] - 2026-07-16

### Fixed
- [fix] eslint
- [fix] import and fix icon component

---


## [0.2.7] - 2026-07-16

### Fixed
- [fix] correct export tooltip component

---


## [0.2.6] - 2026-07-15

### Fixed
- [fix] update homepage in package.json

---


## [0.2.5] - 2026-07-15

### Added
- [add] implementation icons
- [add] implementation tooltip component
- [add] implementarion checkbox component

### Fixed
- [fix] apply lint in icon component
- [fix] update release.sh

---


## [0.2.4] - 2026-07-14

### Added
- [feat] update text area component styles
- [feat] make label prop optional and adjust input styles
- [feat] update Button component with loading state and improve styles

### Fixed
- [fix] simplify Input component layout and adjust styles for consistency

---


## [0.2.3] - 2026-07-13

### Fixed
- [fix] correct export path for CSS file in package.json

---


## [0.2.2] - 2026-07-13

### Added
- [fix] add newline at end of tsup.config.ts

### Fixed
- [fix] update outExtension function to handle ESM format

---


## [0.2.1] - 2026-07-13

### Fixed
- [fix] update CSS file export path in package.json

---


## [0.2.0] - 2026-07-13

### Fixed
- [fix] add CSS file export in package.json

---


## [0.1.1] - 2026-07-13

### Added
- [add] add release script to package.json
- [add] init library

### Fixed
- [fix] update release script to correct lint and unit test numbering and remove E2E tests
- [fix] update button styles to use CSS modules and clean up test cases

---